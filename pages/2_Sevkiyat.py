...
# Helper: Float'tan gelen .0 suffix'ini temizle
def clean_kod(series):
    """Ürün/mağaza kodlarındaki .0 suffix'ini temizle"""
    return series.astype(str).apply(lambda x: x[:-2] if x.endswith('.0') else x)
...
# bölümünün TAMAMINI değiştirmek için kullan
    ...
    if st.button("🚀 HESAPLA", type="primary", width='stretch'):
        ...
        try:
            ...
            df['urun_kod'] = clean_kod(df['urun_kod'])
            df['magaza_kod'] = clean_kod(df['magaza_kod'])
            ...
            depo_df['urun_kod'] = clean_kod(depo_df['urun_kod'])
            depo_df['depo_kod'] = clean_kod(depo_df['depo_kod'])
            depo_df['stok'] = pd.to_numeric(depo_df['stok'], errors='coerce').fillna(0)

            magaza_df = st.session_state.magaza_master.copy()
            magaza_df['magaza_kod'] = clean_kod(magaza_df['magaza_kod'])
            ...
            if paket_sevk_aktif:
                ...
                if (urun_master is not None and
                    ...
                    try:
                        ...
                        paket_info['urun_kod'] = clean_kod(paket_info['urun_kod'])
            ...
            if (urun_master is not None and
                ...
                try:
                    ...
                    urun_m['urun_kod'] = clean_kod(urun_m['urun_kod']).str.strip()
                    urun_m['mg'] = clean_kod(urun_m['mg'].fillna('0')).str.strip()
                    df['urun_kod'] = clean_kod(df['urun_kod']).str.strip()
                    df = df.merge(urun_m, on='urun_kod', how='left')
                    df['mg'] = clean_kod(df['mg'].fillna('0')).str.strip()
                except Exception:
                    df['mg'] = '0'
            else:
                df['mg'] = '0'
            ...
            if not kpi_df.empty and 'mg_id' in kpi_df.columns:
                ...
                kpi_lookup_df['mg_id'] = clean_kod(kpi_lookup_df['mg_id'])
            ...
            depo_stok_merge['depo_kod'] = clean_kod(depo_stok_merge['depo_kod'])
            depo_stok_merge['urun_kod'] = clean_kod(depo_stok_merge['urun_kod'])
            ...
            try:
                ...
                if urun_master is not None and 'paket_ici' in urun_master.columns:
                    ...
                    paket_master['urun_kod'] = clean_kod(paket_master['urun_kod'])
