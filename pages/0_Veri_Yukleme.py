...
# ============================================
# 📊 VERİ YÜKLEME DURUMU TABLOSU - EN ÜSTE ALINDI
# ============================================
...
# ============================================
# 🔍 DETAYLI VERİ İNCELEMESİ
# ============================================
st.subheader("🔍 Detaylı Veri İncelemesi")

loaded_keys = [k for k in data_definitions.keys() if st.session_state.get(data_definitions[k]['state_key']) is not None]
...
if loaded_keys:
    ...
    if selected_data:
        ...
        # Veri önizleme
        with st.expander("📋 Veri Önizleme (İlk 20 satır)", expanded=False):
        ...
        with st.expander("📊 Kolon Bilgileri", expanded=False):
            col_info = pd.DataFrame({
                'Kolon': data.columns,
                'Tip': data.dtypes.astype(str),
                'Boş Değer': data.isnull().sum(),
                'Benzersiz': data.nunique()
            })
...
# VERİ TEMİZLEME
# ============================================
st.subheader("🗑️ Toplu Veri Temizleme")

col1, col2 = st.columns(2)

with col1:
    if st.button("🗑️ Tüm Verileri Temizle", type="secondary", use_container_width=True):
        for definition in data_definitions.values():
            st.session_state[definition['state_key']] = None
        st.success("✅ Tüm veriler temizlendi!")
        time.sleep(0.5)
        st.rerun()

with col2:
    if st.button("🗑️ Sadece Opsiyonel Verileri Temizle", use_container_width=True):
        for key, definition in data_definitions.items():
            if not definition['required']:
                st.session_state[definition['state_key']] = None
        st.success("✅ Opsiyonel veriler temizlendi!")
        time.sleep(0.5)
        st.rerun()
